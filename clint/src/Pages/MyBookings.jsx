...
const MyBookings = () => {
   ...
   const fetchUserBookings = async ()=>{
    try {
      const { data } = await axios.get('/api/bookings/user', {headers: { Authorization: 
 ...
 // Optionally, remove the query param from the URL
  ...
  return (
    <div className='py-28 md:pb-35 px-4 md:pt-32 px-4 md:px-16 lg:px-24 xl:px-32 '>
        ...
        <Tittle title='My Bookings' subTitle='Easily manage your past, current, and upcomming hotel reservations in one place. Plan your trips seamlessly with just a few clicks' align='left'  />
        ...
        <div className='max-w-6xl mt-8 w-full text-gray-800'>
          ...
          {bookings.map((booking)=>(
                  <div key={booking._id} className='grid grid-cols-1 md:grid-cols-1 md:grid-cols-[3fr_2fr_1fr] w-full border-b border-gray-300 py-6 first:border-t'>
                    {/* ---- Hotel Details----- */}
                    <div className='flex flex-col md:flex-row '>
                      <img src={booking.room.images[0]} alt="hotel-img" className='min-md:w-44 rounded shadow object-cover' />

                      <div className='flex flex-col gap-1.5 max-md:md-3 min-md:mT-3 min-md:ml-4 '>
          ...
          {/* ------Payment Status------- */}
            <div className='flex flex-col  justify-center items-start pt-3'>
              ...
              <p className={`text-sm ${booking.isPaid ? 'text-green-500' : 'text-red-500'}`}>{booking.isPaid ? "Paid" : "Unpaid" } 
                    </p>
              </div>
              {!booking.isPaid && (
